...
export function createOutline(overrides = {}) {
    return {
...
export function createEntitySnapshot(name, overrides = {}) {
    return {
        name,
...
export function createRelationSnapshot(overrides) {
    return {
...
export function seedChapter(env, input) {
...
export function seedEntities(env, input) {
    upsertGenerationEntitiesSnapshot(env, input);
}
export function seedForeshadows(env, input) {
...
export function rebuildVolumeRecaps(env, projectId, volumeTitles) {
...
export function seedGeneratedJob(env, input) {
  ...
  `).run(randomUUID(), input.projectId, input.chapterId, input.chapterTitle, input.status ?? 'ready', 1, 'completed', 1, 1, null, '完成', 1, 0, '', '', null, JSON.stringify({}), JSON.stringify(null), input.generatedText, JSON.stringify(null), JSON.stringify(null), JSON.stringify(null), JSON.stringify(null), JSON.stringify(null), JSON.stringify([]), 'quest', '', currentTime, currentTime);
}
