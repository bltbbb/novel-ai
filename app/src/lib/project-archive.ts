...
function assertProjectArchiveShape(value: unknown): asserts value is ProjectArchive {
  ...
  if (
    ...
    throw new Error(
      `归档版本不兼容：当前支持 schemaVersion=1、2 或 ${PROJECT_ARCHIVE_SCHEMA_VERSION}，导入文件为 ${value.schemaVersion}`,
    );
  }
...
export async function buildProjectArchive(projectId: Id): Promise<ProjectArchive> {
  ...
  const [chapters, volumes, bookOutlines, volumeOutlines, chapterBeats, entities, foreshadows, snapshots, ideaCards] = await Promise.all([
...
export async function importProjectArchive(archive: ProjectArchive) {
  ...
  const normalizedChapterBeats =
    parsedArchive.schemaVersion >= PROJECT_ARCHIVE_SCHEMA_VERSION
      ? remapChapterBeats(
